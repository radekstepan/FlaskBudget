...
class AccountsTestCases(unittest.TestCase):
    ...
    def test_edit_accounts(self):
        ...
        # modify balance
        rv = self.app.post('/account/edit', data=dict(account=1, balance=500), follow_redirects=True)
        assert 'Balance modified' in rv.data

        # modify balance
        rv = self.app.post('/account/edit', data=dict(account=2, balance=-101.70), follow_redirects=True)
        assert 'Balance modified' in rv.data
        ...
        ''' in rv.data
        assert '''
            <span class="amount">&minus; &pound;101.70</span>
            <a>Credit Card</a>
        ''' in rv.data
